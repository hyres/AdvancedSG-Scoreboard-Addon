...
public class ScoreboardHelper
{
    ...
    public ScoreboardHelper(final Scoreboard scoreBoard2) {
        this.title = "Lobby";
        this.ttl = "Lobby";
        this.list = new ArrayList<ScoreboardText>();
        this.tag = "PlaceHolder";
        this.lastSentCount = -1;
        Preconditions.checkState(this.title.length() <= 32, (Object)"title can not be more than 32");
        if (Game.getStageName() != null) {
            if (Game.getStageName().equals("Lobby")) {
                this.title = Main.plugin.getConfig().getString("Scoreboard.LOBBY.title").replaceAll("%game_time%", Utils.getTimer()).replaceAll("&", "§");
            }
            else if (Game.getStageName().equals("Pre-Game")) {
                this.title = Main.plugin.getConfig().getString("Scoreboard.PREGAME.title").replaceAll("%game_time%", Utils.getTimer()).replaceAll("&", "§");
            }
            else {
                this.title = "Lobby";
            }
        ...
        this.tag = ChatColor.translateAlternateColorCodes('&', this.title);
    ...
    public void update(final Player player) {
        if (Game.getStageName() != null) {
            if (Game.getStageName().equals("Lobby")) {
                this.ttl = Utils.replacer(Main.plugin.getConfig().getString("Scoreboard.LOBBY.title").replaceAll("%game_time%", Utils.getTimer()).replaceAll("&", "§"), player);
            }
            else if (Game.getStageName().equals("Pre-Game")) {
                this.ttl = Utils.replacer(Main.plugin.getConfig().getString("Scoreboard.PREGAME.title").replaceAll("%game_time%", Utils.getTimer()).replaceAll("&", "§"), player);
            }
            else if (Game.getStageName().equals("Game")) {
                this.ttl = Utils.replacer(Main.plugin.getConfig().getString("Scoreboard.LIVEGAME.title").replaceAll("%game_time%", Utils.getTimer()).replaceAll("&", "§"), player);
            }
            else if (Game.getStageName().equals("Pre-Deathmatch")) {
                this.ttl = Utils.replacer(Main.plugin.getConfig().getString("Scoreboard.PREDEATHMATCH.title").replaceAll("%game_time%", Utils.getTimer()).replaceAll("&", "§"), player);
            }
            else if (Game.getStageName().equals("Deathmatch")) {
                this.ttl = Utils.replacer(Main.plugin.getConfig().getString("Scoreboard.DEATHMATCH.title").replaceAll("%game_time%", Utils.getTimer()).replaceAll("&", "§"), player);
            }
            else if (Game.getStageName().equals("Restarting")) {
                this.ttl = Utils.replacer2(Main.plugin.getConfig().getString("Scoreboard.RESTARTING.title").replaceAll("%game_time%", Utils.getTimer()).replaceAll("&", "§"), player);
            }
            else {
                this.ttl = "Lobby";
            }
        ...
        for (int sentCount = 0; sentCount < this.list.size(); ++sentCount) {
            ...
            this.objective.setDisplayName(this.ttl);
        }
